...
// Main controller class in the game logic. Knows and as access to the players, the door, the keyholders, the levelpart components, the levelparts and
// the state of an actual game.
public class Level {
	private int width;				// level size
	private int height;
	private LevelPart[][] parts;			// 2D array of levelparts (aka puzzle elements)
	private KeyHolder[] keyHolders;			// array of keyholders
	private Door door;				// exit point of given level
	private LevelState state;			// the state of a given level represents if the keys are collected yet or is the level finished
	private Player player1;				// player spawn points
